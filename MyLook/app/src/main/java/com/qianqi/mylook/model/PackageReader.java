...
public class PackageReader {
    ...
    public void loadAllPackages(boolean ignoreIcon, OnLoadListener mOnLoadListener) {
...
//			L.v("Processing package "+p.packageName);
            mCurrentPackage = loadPackage(false, p);
...
//                    L.d("gray app:"+p.packageName);
        ...
        mOnLoadListener.onLoadPackageInfo(mResult, whiteApps, graySystemApps);
        if (ignoreIcon)
            return;
...
//		}
    ...
    public EnhancePackageInfo loadPackage(boolean ignoreIcon, String packageName) {
        EnhancePackageInfo mCurrentPackage = null;
        try {
            PackageInfo p = null;
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
            ...
            mCurrentPackage = loadPackage(!ignoreIcon, p);
...
//	}
    ...
    static boolean isPersistentApp(PackageInfo p) {
        // You'd think that it would be possible to determine the
        // system status of packages that do not have a application,
        // as rare as that may be, but alas, it doesn't look like it.
        // Of course, in those cases there'd be no receivers for us
    ...
    static boolean hasLauncher(PackageInfo pi) {
        Intent resolveIntent = new Intent(Intent.ACTION_MAIN, null);
        resolveIntent.setPackage(pi.packageName);
        resolveIntent.addCategory(Intent.CATEGORY_LAUNCHER);
        PackageManager pManager = MainApplication.getInstance().getPackageManager();
