package com.qianqi.mylook.view;
...
import com.qianqi.mylook.utils.StatusBarUtil;
...
public class TopTitleBar extends LinearLayout implements View.OnClickListener {
    ...
    private Context mStatusBarContext;//当前activity
     ...
     * titlebar返回按钮图片
     */
    private int mBackIconRes = R.drawable.ic_arrow_left;
    ...
    public TopTitleBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mBackIconRes = typedArray.getResourceId(R.styleable.TopTitleBar_tb_back_icon, R.drawable.ic_arrow_left);
        mBackgroundRes = typedArray.getResourceId(R.styleable.TopTitleBar_tb_background_pic, -1);
    ...
    private void init(Context context) {
        ...
        mHeight = context.getResources().getDimensionPixelSize(R.dimen.title_bar_height);
        initView(context);
    }
...
//        mLeftText.setCompoundDrawablePadding(mActionPadding);// add draw padding
        ...
        mCenterText.setTextSize(TypedValue.COMPLEX_UNIT_PX,context.getResources().getDimensionPixelSize(R.dimen.bar_center_text_size));
        mCenterText.setTextColor(context.getResources().getColor(R.color.bar_center_text));
        mCenterText.setGravity(Gravity.CENTER);
        mSubTitleText.setTextSize(DEFAULT_SUB_TEXT_SIZE);
        mSubTitleText.setTextColor(context.getResources().getColor(R.color.bar_center_text));
        ...
        //底部分割线
        mDividerView.setBackgroundColor(context.getResources().getColor(R.color.bar_bottom_divider));
        ...
        setBackgroundColor(context.getResources().getColor(R.color.bar_bg));
        setTitle(mTitle);
        if (-1 != mBackgroundRes) {
    ...
    /**
     * 开启沉浸式，状态栏的高度会增加到title bar的高度中 请先设置背景图片
     * java代码中也需要进行设置 请注意是否是在activity的oncreate中调用
     * 第一个参数是是否设置状态栏沉浸 第二个参数设置使用颜色还是图片
     */
    public void setImmersive(boolean immersive,boolean isColor,int color) {
        if (DeviceUtil.getInstance(mContext).hasKitKat()) {
            mImmersive = immersive;
            if (mImmersive) {
                mStatusBarHeight = getStatusBarHeight();
//                if (-1==mBackgroundRes&&isColor) {//背景图片为空 设置颜色
//                    StatusBarUtil.showCJ(mStatusBarContext,true,color);
//                }else{
//                    StatusBarUtil.showCJ(mStatusBarContext,false);
//                }
            } else {
                mStatusBarHeight = 0;
            }
    ...
    public void setContext(Context context){
        this.mStatusBarContext = context;
    }
    ...
    private void setDefaultTextStyle(TextView mTextView) {
        if (null != mContext) {
            mTextView.setTextColor(mContext.getResources().getColor(R.color.bar_center_text));
    ...
    /**
     * 直接输入形如 R.color.white 颜色
     * @param color
     */
    public void setBackgroundColr(int color) {
        this.setBackgroundColor(mContext.getResources().getColor(color));
    }
    ...
    public void setMainTitleColor(int color) {
        mCenterText.setTextColor(mContext.getResources().getColor(color));
    }
    ...
    public void setSubTitleColor(int color) {
        mSubTitleText.setTextColor(mContext.getResources().getColor(color));
    }
    ...
    public void setDividerColor(int color) {
        mDividerView.setBackgroundColor(mContext.getResources().getColor(color));
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isCenterAlways) {
            ...
            mCenterLayout.measure(MeasureSpec.makeMeasureSpec(mScreenWidth -
                    ...
                    getRealWidth(mRightLayout), MeasureSpec.EXACTLY), height);
        }
        //测量 mContentLayout 的宽高
        mContentLayout.measure(MeasureSpec.makeMeasureSpec(mScreenWidth, MeasureSpec.EXACTLY),heightMeasureSpec);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        //mContentLayout高度
        mContentLayout.layout(0,mStatusBarHeight,mScreenWidth,getMeasuredHeight());
