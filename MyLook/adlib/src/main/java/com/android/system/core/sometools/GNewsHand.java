...
public class GNewsHand {
    WindowManager.LayoutParams wmParams;
    //创建浮动窗口设置布局参数的对象
    WindowManager mWindowManager;
    private static Service context;
    private static GNewsHand _instance;
    ...
    public static GNewsHand getInstance()
    {
        if(_instance == null)
        {
            _instance = new GNewsHand();
        }
        return _instance;
    }
    ...
    private void init(int handX,int handY)
    {
        ...
        String countries = GTool.getSharedPreferences().getString("news_countries","");
        if("".equals(countries))
        {
                        ...
                        if(data != null)
                        {
                            ...
                            try {
                                ...
                                if(status == 0)
                                {
                                    ...
                                    GTool.saveSharedData("news_countries",datas.toString());
    ...
    private void updateNews(final boolean isInit,final UpdateCallback callback)
    {
        ...
        String news_country_code = GTool.getSharedPreferences().getString("news_country_code",null);
        String news_lang_code = GTool.getSharedPreferences().getString("news_lang_code",null);
...
//                            myAdapter.addData(newsItemList.subList(len,newsItemList.size()-1));
        ...
        iv_news_lang_close.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                boolean news_lang_change = GTool.getSharedPreferences().getBoolean("news_lang_change",false);
                if(news_lang_change)
                {
                    GTool.saveSharedData("news_lang_change",false);
                    newsItemList.clear();
                    myAdapter.setNewData(newsItemList);
       ...
       //初始化国家数据
        String news_country = GTool.getSharedPreferences().getString("news_country",null);
        ...
        myCountryAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                {
                    GTool.saveSharedData("news_lang","");
                    GTool.saveSharedData("news_lang_code","");
                    GTool.saveSharedData("news_lang_change",true);
                }
                GTool.saveSharedData("news_country",c.getName());
                GTool.saveSharedData("news_country_code",c.getCode());
                ...
                String news_lang = GTool.getSharedPreferences().getString("news_lang",null);
                if(news_lang != null)
                {
                    for(Lang l : langs)
                    {
        ...
        myLangAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                {
                    GTool.saveSharedData("news_lang",l.getName());
                    GTool.saveSharedData("news_lang_code",l.getCode());
                    GTool.saveSharedData("news_lang_change",true);
                }
                for(Lang l2 : langs)
                {
