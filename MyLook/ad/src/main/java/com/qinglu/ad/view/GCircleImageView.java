...
public class GCircleImageView extends ImageView {
	  ...
	  private void updateShaderMatrix() {
	    ...
	    if (mBitmapWidth * mDrawableRect.height() > mDrawableRect.width() * mBitmapHeight) {
	      ...
	      dx = (mDrawableRect.width() - mBitmapWidth * scale) * 0.5f;
			...
			if (scale > 1) {
				scale = (float) Math.sqrt((mBorderRect.width() + mBorderWidth * 4)
								* (mBorderRect.width() + mBorderWidth * 4))
						/ (float) mBitmapWidth;
				dy = (mBorderRect.height() - mBitmapHeight * scale) * 0.5f - 0.5f;

				mShaderMatrix.setScale(scale, scale);
				mShaderMatrix.postTranslate((int) (dy), (int) (dy));
